...
name=HungerGamesPE
description=A Hunger Games plugin for MCPE servers
version=1.0
author=Exxarion
class=hungergames
apiversion=12
...
class hungergames implements Plugin{
	...
	public function init() {
		...
		$this->api->console->register("hg", "Main Hunger games command.", array($this, "command"));
		$this->api->console->register("kit", "Hunger Games Kits.", array($this, "command"));
		...
		switch ($event) {
			...
			case "player.spawn":
				if (!$this->switch["server.gate"]) {
					...
					$data->sendChat("+========================================+");
					$data->sendChat(" ");
					$data->sendChat("A game is currently in progress.");
					$data->sendChat("Please join later.");
					$data->sendChat(" ");
					$data->sendChat("+========================================+");
					$data->close("game is going on.", false);
					$this->broadcast("".$data->username." has left the game.");
					return false;
				}
	...
	public function loserProcess($data, $event) {
		switch($event) {
			case "player.death":
				...
				if ($nop > 1) {
					$this->broadcast(FORMAT_AQUA."".$dead." was ".$reason.".".$nop." players remaining.");
				} else {
			...
			case "player.quit":
				...
				$nop = count($this->players);
				if ($nop > 1) {
					$this->broadcast(FORMAT_AQUA."A player has disconnected.".$nop." players remaining.");
				} else {
					$winner = array_shift($this->players);
					$this->schedule(1, "gameFinish", $winner->username);
				}
			...
			case "hg":
				...
				switch($mode) {
					case "run":
						...
						console("");
						$this->api->chat->broadcast(FORMAT_AQUA."Game started!");
						console("");
					...
					case "start":
						if ($this->status !== "lobby") {
							$output .= "This command is unavailable now.\n";
							break;
						}
					...
					case "stop":
						if ($this->status === false) {
							$output .= FORMAT_YELLOW."game not started!\n";
							break;
						}
						$this->gameStop();
						break;
					case "marker":
						...
						if ($bool) {
							...
							$output .= "Markers placed!";
						} else {
							$this->breakPointMarker();
							$output .= "Markers removed";
						}
						break;
					case "state":
					case "status":
					...
					case "settime":
						...
						$output .= "\"".$mode."\" time is set to ".$time.".\n";
						break;
					case "setday":
					case "locktime":
						$mode = array_shift($params);
						if (in_array($mode, array("day", "night", "sunset", "sunrise"))) {
							...
							$this->writeConfig();
							$output .= "Set lock-time to \"$mode\".\n";
						} else {
							$output .= "Failed to set time.\n";
						}
						break;
					case "prize":
						$amount = (int) array_shift($params);
						if ($amount = "" or $amount < 0) {
							$output .= "Usage: /hg prize <amount>\n";
							break;
						}
					...
					case "protect":
						...
						if ($bool) {
							$output .= "Turned on world protection for tournaments.\n";
							$this->config["world-protect"] = true;
						} else {
							$output .= "Turned off the protection for tournaments.\n";
					...
					case "addfield":
						...
						if ($this->isAlnum($field) === false) {
							$output .= FORMAT_YELLOW."You need to use English.";
							break;
						}
						if (empty($levelname)) {
							$levelname = false;
						...
						if ($this->fieldExists($field)) {
							$output .= FORMAT_YELLOW."\"".$field."\" already exists!\n";
							break;
						}
						$this->config["field"][$field] = array("lobby" => array(), "start" => array(), "level" => $levelname);
						$this->writeConfig();
						$output .= FORMAT_AQUA." \"".$field."\" creation field succeeded! Next, you must set a lobby point and start points of the field.\n";
					...
					case "rmfield":
						$field = (string) array_shift($params);
						if (!$this->fieldExists($field)) {
							$output .= FORMAT_YELLOW." \"".$field."\" doesn't exist!\n";
							break;
						}
						unset($this->config["field"][$field]);
						$output .= FORMAT_AQUA."Field, \"".$field."\", removed successfully!\n";
						$this->writeConfig();
						break;
					case "fieldinfo":
						$field = (string) array_shift($params);
				...
				switch ($mode) {
					...
					case "add":
						...
						if ($this->kit->add($name, $price, $level) === true) {
							$output .= FORMAT_DARK_AQUA."Added $name kit!\n";
						} else {
							$output .= FORMAT_YELLOW."Failed to add $name !\n";
						}
						break;
					case "removekit":
					case "remove":
					...
					case "rm":
						...
						if ($this->isAlnum($kitname) === false) {
							$output .= FORMAT_YELLOW."You need to use English for kit name.";
							break;
						}
						if ($this->kit->remove($kitname)) {
							$output .= FORMAT_DARK_AQUA."Removed \"$kitname\"!\n";
						} else {
							$output .= FORMAT_YELLOW."Failed to remove \"$kitname\"!\n";
						}
						break;
					case "list":
					...
					case "additem":
						...
						if ($this->kit->get($kitname) === false) {
							$output .= FORMAT_YELLOW."The kit \"$kitname\" doesn't exist.\n";
							break;
						}
						if (!isset(Block::$class[$id]) and !isset(Item::$class[$id])) {
							$output .= FORMAT_YELLOW."WARNING: The item id \"$id\" could be incorrect.\n";
						}
						if ($meta === null) {
							$meta = 0;
						}
						...
						if ($this->kit->editItem("add", $kitname, $sets)) {
							$output .= FORMAT_DARK_AQUA."Added items to \"$kitname\"!\n";
						} else {
							$output .= FORMAT_YELLOW."Failed to add items to \"$kitname\".\n";
						}
						$this->kit->showKitInfo($kitname);
						break;
					case debug:
		...
		switch ($cmd) {
			case "kit":
				...
				switch ($kitname) {
					...
					case "help":
						$output .= "Usage: /kit <kit name> ......... buy a kit\n";
						$output .= "Usage: /kit list ......... show a kit list\n";
						break;
					case "list":
					case "ls":
						$this->kit->showList($output);
						$output .= "how to buy: /kit <kit name>\n";
						break;
					default:
	...
	public function editField($mode, $params, &$output) {
		$field = array_shift($params);
		if (!$this->fieldExists($field)) {
			$output .= FORMAT_YELLOW."The field doesn't exist!".FORMAT_RESET.": \"".FORMAT_GREEN."".$field.FORMAT_RESET."\"\n";
		...
		switch ($mode) {
			...
			case "addpoint":
				...
				$z = array_shift($params);
				if (!is_numeric($x) or !is_numeric($y) or !is_numeric($z)) {
					$output .= "Usage: /hg addlobby <field name> <x> <y> <z>\n";
					break;
				}
				$x = (float) $x;
				$y = (float) $y;
	...
	public function gameInvincible() {
		...
		if ($this->setPlayers() === false) {
			$this->broadcast(FORMAT_YELLOW."Game did not start!");
			$this->broadcast(FORMAT_YELLOW."The must be 2 or more people online.");
			console("");
			$this->gameSuspend();
			return;
		}
		...
		$this->broadcast(FORMAT_YELLOW."There are currently ".$nop." players competing.");
		$this->broadcast(FORMAT_YELLOW."Be the last one standing and good Luck!");
		$this->broadcast(" ");
		$this->broadcast(FORMAT_YELLOW."+==============================================+");
	...
	public function gamePlay() {
		$this->status = "play";
		$this->tool("pvp", true);
		$this->broadcast(FORMAT_YELLOW."You are no longer invincible.".FORMAT_RESET."");
	...
	public function gameFinish($winner = false) {
		...
		if ($winner !== false) {
			...
			$this->broadcast(FORMAT_YELLOW."+==============================================+");
			$this->broadcast(" ");
			$this->broadcast(FORMAT_YELLOW."This game is suspended!");
			$this->broadcast(FORMAT_YELLOW."Remnants list:");
			$this->broadcast(FORMAT_YELLOW.$message);
			$this->broadcast(" ");
			$this->broadcast(FORMAT_YELLOW."+==============================================+");
		}
		$this->schedule(5, "broadcast", "All players will be disconnected.");
	...
	public function gameStop() {
		$this->broadcast(FORMAT_RED."Game stopped! Everyone will be kicked!");
		$this->cancelAllSchedules();
		$this->resetParams();
                $this->schedule(3, "kick", array($player, "game stopped."));
                $this->api->console->run("hg" . "run")
	}
	...
	public function tool($tool, $params) {
		switch($tool) {
			...
			case "backup":
				...
				if ($bool) {
					console(" The world data is has been backed up.");
					$this->backupChest();
				} else {
					$this->restoreWorld();
				}
	...
	public function setPlayers() {
		...
		if (count($players) !== 0) {
			console("");
			console(FORMAT_AQUA."Player list:");
			console();
			foreach ($players as $player) {
				if ($player->gamemode == 0) { //creative
					...
					$player->sendChat("You are not Survival mode!");
					$player->sendChat("You will be kicked.");
	...
	public function setField($field = false) {
		if (count($this->config["field"]) == 0) {
			console(FORMAT_YELLOW."There is no field data! You have to add a field at first.");
		...
		if ($field === false) {
			...
			if ($field === false) {
				console(FORMAT_YELLOW."There is no proper field data! You have to add a field at first.");
				console ("[HungerGames] Usage: /hg addfield <field name>");
				return false;
			}
			...
			console(FORMAT_YELLOW."\"".$field."\" doesn't exist!");
			return false;
		}
		if (!$this->testField($field)) {
			console(FORMAT_YELLOW."\"".$field."\" has some incomplete parts!");
			return false;
		}
		$this->broadcast("Next stage is selected to \"".$field."\"!");
		return true;
	}
	...
	public function getFieldLevel() {
		...
		if ($level === false) {
			console(FORMAT_YELLOW."level: \"".$field."\" doesn't exist!");
			$this->gameStop();
		}
		return $level;
	}
	...
	public function showState(&$output) {
		$output .= FORMAT_AQUA."The tournament's state:".FORMAT_RESET."   [Remnants: ".FORMAT_GREEN."".count($this->players).FORMAT_RESET."/".FORMAT_GREEN."".count($this->score).FORMAT_RESET." players]\n";
	...
	public function showAccountInfo(Player $player) {
		...
		$player->sendChat("+=========================================+");
		$player->sendChat(" ");
		$player->sendChat("Hunger games server:");
		$player->sendChat("             ".$server);
		$player->sendChat("name: ".$player->username."        level: ".$rec["level"]."  exp: ".$rec["exp"]."/".$quota);
		$player->sendChat(" win: ".$rec["win"]."  kill: ".$rec["kill"]."  death: ".$rec["death"]."  kill/death ratio: ".$kd."  coins: ".$coin);
		$player->sendChat(" ");
		$player->sendChat("+=========================================+");
	}
	...
	public function placePointMarker() {
		...
		foreach ($this->config["field"] as $field => $data) {
			if ($data["level"] === false) {
				...
				if ($level === false) {
					console(FORMAT_YELLOW."".$field."'s level doesn't exist!".FORMAT_RESET."");
					continue;
				}
			}
			$line1 = "Field: " . $field;
	...
	public function createConfig() {
		$config = array(
				...
				"announce" => array(
						...
						"Your inventory will be emptied when game is started",
						"You should not carry any items in the game.",
						"This game is not the team system.",
						"Whoever survives at the end will be the winner.",
						"You can buy Kits by using /kit.",
						"Coins have been provided when you joined.",
				),
				"field" => array(),
		);
...
class KitManager{
	...
	public function editItem($mode, $param, $sets) {
		switch ($mode) {
			case "add":
				...
				console(FORMAT_RED."cannot add items anymore.");
				return false;
			case "remove":
				$slot = (Int) $param;
				break;
	...
	public function showKitInfo($kitname) {
		$kit = $this->get($kitname);
		if ($kit === false) {
			console(FORMAT_YELLOW."The kit \"$kitname\" doesn't exist!");
		}
		console("");
